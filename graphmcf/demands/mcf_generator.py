...
class MCFGenerator:
    def __init__(
        ...
        *,
        ...
        p_ER: float = 0.5,
        distribution: str = "normal",              # тип распределения и для initial, и для добавляемых рёбер
        median_weight_for_initial: int = 50,
        var_for_initial: int = 100,
        # --- делители для добавляемых рёбер (от capacity) ---
    ...
    def _draw_new_weight(self, old_w: float, mode: str, median: int, var: int) -> int:
        """
        Дискретная усечённая «нормаль» на {1, ..., 2*median}:
          mode=">" → выбрать новое значение > old_w (adversarial)
          mode="<" → выбрать новое значение < old_w (friendly)
        """
        if self.dist != "normal":
    ...
    def generate(
        ...
        """
        Основной алгоритм. ВАЖНО:
        - initial demands генерим через graph.generate_initial_demands(...) с параметрами генератора:
              p_ER, dist, median_weight_for_initial, var_for_initial
        - параметры ДЛЯ ДОБАВЛЯЕМЫХ рёбер считаем от capacity-графа, как раньше:
              median_for_weights = median(capacity)//median_div
              var_for_weights    = median(capacity)//var_div
          при этом тип распределения синхронизирован через self.dist.
        """
        # 0) initial demands — управляем сверху
        start = time.time()
        ...
        # 1) параметры для добавляемых рёбер из capacity (как было)
        ...
        # история для анализа (как согласовано)
        ...
        # прикладываем историю для анализа
